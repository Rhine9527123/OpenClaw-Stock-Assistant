...
> 基于 OpenClaw 的**低 Token 股票投资助手**  
> 自动抓取行情 → 技术指标分析 → 生成晨报 → 飞书推送，**单次晨报仅需 100 tokens，比纯 AI 生成节省 97%**。

---

## 这是什么？

**一个算法驱动 + 轻量 AI 辅助的股票晨报自动生成系统。**

用 Python 算法完成数据采集、技术指标计算和报告结构生成，只在最后一步用极少量的 AI Token 做文本润色，自动生成带可视化图表的每日晨报，推送到飞书。

- 输入：股票代码（如 `588000` 科创50ETF）
- 输出：数据摘要 + 技术指标解读 + 趋势判断 + 可视化图表
- 成本：**单次晨报 100 tokens，单日全部任务 921 tokens**

**为什么做这个项目？**

市面上大多数"AI 投资助手"完全依赖大模型生成分析，一次晨报就要消耗 ~3100 tokens。这个项目证明：**用算法替代 97% 的 AI 生成工作**，不仅成本极低，而且基于真实数据的分析比纯 AI"一本正经地胡说"更靠谱。
...
| 特性 | 说明 |
|------|------|
| **97% Token 节省** | 算法替代大部分 AI 生成工作，单次晨报仅需 100 tokens（纯 AI 方案需 ~3100 tokens）|
...
| 维度 | 本项目（数据驱动） | 大模型方案 |
|------|------------------|-----------|
| 成本 | **单次 100 tokens**（算法为主，AI 仅做润色） | 每次分析 ~3100 tokens，日均 0.5-2 元 |
...
| 方案 | 单次晨报 | 单日全部任务 | 月均成本（按 22 个交易日）|
|------|---------|-------------|------------------------|
| **纯 AI 生成** | ~3100 tokens | ~68200 tokens | ~15-20 元 |
| **本项目（算法 + 轻量 AI）** | **~100 tokens** | **~921 tokens** | **~0.5 元** |
| **节省比例** | — | — | **~97%** |

**结论：** 晨报这种**结构化、数据密集型**任务，用算法替代大部分 AI 生成工作，比纯大模型方案更靠谱、更便宜、更快。
